import { Task, User, CreateTaskInput, UpdateTaskInput, Priority } from '@task-tracker/types';
import { TABLES } from './db/dynamodb.js';
import { 
  ScanCommand, 
  QueryCommand, 
  GetItemCommand, 
...
const resolvers: Resolvers = {
  ...
  Mutation: {
    createTask: async (_: unknown, { input }: { input: CreateTaskInput }): Promise<Task> => {
      console.log(`[createTask Mutation] Creating task with input:`, input);
      try {
        // Get the creator user
        const creatorCommand = new GetItemCommand({
          TableName: TABLES.USERS,
          Key: { id: toDynamoDBValue(input.createdById) },
        });
        const creatorResponse = await client.send(creatorCommand);
        
        if (!creatorResponse.Item) {
          throw new Error(`Creator user with ID ${input.createdById} not found.`);
        }

        const creator = convertDynamoItemToUser(creatorResponse.Item);
        const now = new Date().toISOString();

        const task: Task = {
          id: 1, // This should be generated by your ID generation strategy
          ...input,
          createdAt: now,
          updatedAt: now,
          createdBy: creator,
        ...
        const command = new PutItemCommand({
          TableName: TABLES.TASKS,
          Item: {
            id: toDynamoDBValue(task.id),
            title: toDynamoDBValue(task.title),
            description: toDynamoDBValue(task.description),
            status: toDynamoDBValue(task.status),
            dueDate: toDynamoDBValue(task.dueDate),
            scheduledDate: toDynamoDBValue(task.scheduledDate),
            completionDate: toDynamoDBValue(task.completionDate),
            priority: toDynamoDBValue(task.priority),
            isRecurring: toDynamoDBValue(task.isRecurring),
            assignedToId: toDynamoDBValue(task.assignedToId),
            createdById: toDynamoDBValue(task.createdById),
            tenantId: toDynamoDBValue(task.tenantId),
            createdAt: toDynamoDBValue(task.createdAt),
            updatedAt: toDynamoDBValue(task.updatedAt)
          }
        });

        await client.send(command);
        console.log('[createTask Mutation] Task created successfully:', task);
        return task;
      } catch (error) {
